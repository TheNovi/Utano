...
class Select(Scene):
	...
	def activate(self):
		...
		self.filter(self.e_search.get(), self.ut.actual_i)
	...
	def filter(self, reg='', i=0):
		...
		for s in self.ut.all_songs:
			...
			if tkinter.re.match(r'.*{}.*'.format(tkinter.re.escape(reg)), f, tkinter.re.IGNORECASE):
				...
				if len(f) > width:
					width = len(f)
		self.l_info['text'] = f'{len(self.sub_select)}/{len(self.ut.all_songs)}'
	...
	def selected(self):  # Selected by mouse
		if self.manager.activated == self:
			self.ut.apply_song_select(self.sub_select)
			self.manager.escape()

	def activated(self):  # Selected by <Return>
		self.manager.ut.apply_song_select(self.sub_select)
		self.manager.escape()
